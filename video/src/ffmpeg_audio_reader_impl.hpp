...
public:
  ...
  bool open(const std::string &url) {
    ::deepir::video::ffmpeg::init_library();
    int ret = 0;
    this->close();
    ...
    if (url_scheme == "file") {
      ...
      if (ext == ".pcm_mulaw") {
        ...
        if (ret != 0) {
          DLOG_ERROR("av_dict_set failed:{}",
                     ::deepir::video::ffmpeg::errno_to_str(ret));
          return false;
        }
      }
    }
    ...
    if (ret != 0) {
      DLOG_ERROR("avformat_open_input failed:{}",
                 ::deepir::video::ffmpeg::errno_to_str(ret));
      return false;
    }
    ...
    if (ret < 0) {
      DLOG_ERROR("avformat_find_stream_info failed:{}",
                 ::deepir::video::ffmpeg::errno_to_str(ret));
      return false;
    }
