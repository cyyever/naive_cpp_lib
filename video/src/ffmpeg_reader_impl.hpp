...
template <bool decode_frame> class reader_impl : private deepir::runnable {
public:
  ...
  bool open(const std::string &url) {
    ...
    if (!input_ctx) {
      DLOG_ERROR("avformat_alloc_context failed");
      return false;
    }
    ...
    if (url_scheme == "rtsp") {
      ...
      if (ret != 0) {
        DLOG_ERROR("av_dict_set failed:{}", errno_to_str(ret));
        return false;
      }
    ...
    if (ret != 0) {
      DLOG_ERROR("avformat_open_input failed:{}", errno_to_str(ret));
      return false;
    }

    ret = avformat_find_stream_info(input_ctx, nullptr);
    if (ret < 0) {
      DLOG_ERROR("avformat_find_stream_info failed:{}", errno_to_str(ret));
      return false;
    }
    ...
    if (ret < 0) {
      DLOG_ERROR("av_find_best_stream failed");
      return false;
    }
    ...
    */
    ...
    if (!codec) {
      DLOG_ERROR("can't find decoder for {}", url);
      return false;
    }
    ...
    if (!decode_ctx) {
      DLOG_ERROR("avcodec_alloc_context3 failed");
      return false;
    }

    ret = avcodec_parameters_to_context(decode_ctx, video_stream->codecpar);
    if (ret < 0) {
      DLOG_ERROR("avcodec_parameters_to_context failed:{}", errno_to_str(ret));
      return false;
    }

    ret = avcodec_open2(decode_ctx, nullptr, nullptr);
    if (ret < 0) {
      DLOG_ERROR("avcodec_open2 failed:{}", errno_to_str(ret));
      return false;
    }
    ...
    if (video_width <= 0 || video_height <= 0) {
      DLOG_ERROR("invalid video size [{} * {}]", video_width, video_height);
      return false;
    }

    avframe = av_frame_alloc();
    if (!avframe) {
      DLOG_ERROR("av_frame_alloc failed");
      return false;
    }
    ...
    if (url_scheme == "rtsp") {
      frame_buffer = std::make_unique<deepir::thread_safe_linear_container<
          std::vector<std::pair<int, frame>>>>(100);
      packet_buffer = std::make_unique<deepir::thread_safe_linear_container<
          std::vector<std::pair<int, std::shared_ptr<AVPacket>>>>>(100);
      start();
    }
  ...
  std::pair<int, std::shared_ptr<AVPacket>> next_packet() {
    if (!has_open()) {
      DLOG_ERROR("video is not opened");
      return {-1, {}};
    }
  ...
  std::optional<std::array<size_t, 2>> get_frame_rate() {
    ...
    auto res = av_guess_frame_rate(input_ctx, input_ctx->streams[stream_index],
                                   nullptr);

    if (res.den <= 0 || res.num <= 0) {
      DLOG_ERROR("invalid frame rate [{} {}]", res.num, res.den);
      return {};
    }
...
private:
  ...
  void run() override {
    ...
    while (!needs_stop()) {
      if constexpr (decode_frame) {
        ...
        if (res.first <= 0) {
          DLOG_ERROR("get frame failed,thread exit");
          break;
        }
      } else {
        auto res = get_packet();
        ...
        if (res.first <= 0) {
          DLOG_ERROR("get packet failed,thread exit");
          break;
        }
      }
    }
  ...
  int get_packet(AVPacket &packet) {
    ...
    while (true) {
      ...
      if (ret != 0) {
        DLOG_ERROR("av_read_frame failed:{}", errno_to_str(ret));
        return -1;
      }
  ...
  std::pair<int, std::shared_ptr<AVPacket>> get_packet() {
    if (!has_open()) {
      DLOG_ERROR("reader is not opened");
      return {-1, nullptr};
    }

    auto packet = av_packet_alloc();
    if (!packet) {
      DLOG_ERROR("av_packet_alloc failed");
      return {-1, nullptr};
    }
    ...
    if (!packet2) {
      DLOG_ERROR("av_packet_clone failed");
      return {-1, nullptr};
    }
  ...
  std::pair<int, frame> get_frame() {
    ...
    while (true) {
      ...
      if (ret != AVERROR(EAGAIN)) {
        DLOG_ERROR("avcodec_receive_frame failed:{}", errno_to_str(ret));
        return {-1, {}};
      }
      ...
      if (ret != 0 && ret != AVERROR(EAGAIN) && ret != AVERROR_INVALIDDATA) {
        DLOG_ERROR("avcodec_send_packet failed:{}", errno_to_str(ret));
        return {-1, {}};
      }
    }
    ...
    if (!sws_ctx) {
      DLOG_ERROR("sws_getContext failed");
      return {-1, {}};
    }
    ...
    if (ret <= 0) {
      DLOG_ERROR("av_image_fill_pointers failed:{}", errno_to_str(ret));
      return {-1, {}};
    }
    ...
    if (ret <= 0) {
      if (ret < 0) {
        DLOG_ERROR("sws_scale failed:{}", errno_to_str(ret));
      } else {
        DLOG_ERROR("sws_scale failed");
      }
      return {-1, {}};
    }

    DLOG_DEBUG("new frame");
    return {1, new_frame};
  }
...
private:
  ...
  std::unique_ptr<
      deepir::thread_safe_linear_container<std::vector<std::pair<int, frame>>>>
      frame_buffer;
  std::unique_ptr<deepir::thread_safe_linear_container<
