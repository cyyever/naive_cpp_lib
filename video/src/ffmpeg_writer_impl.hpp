...
public:
  ...
  bool open(const std::string &url, const std::string &format_name,
    ...
    if (ret < 0) {
      DLOG_ERROR("avformat_alloc_output_context2 failed:{}", errno_to_str(ret));
      return false;
    }
    ...
    if (!codec) {
      DLOG_ERROR("can't find encoder for url {}", url);
      return false;
    }

    output_stream = avformat_new_stream(output_ctx, nullptr);
    if (!output_stream) {
      DLOG_ERROR("avformat_new_stream failed");
      return false;
    }
    ...
    if (!(output_ctx->oformat->flags & AVFMT_NOFILE)) {
      ...
      if (ret < 0) {
        DLOG_ERROR("avio_open failed:{}", errno_to_str(ret));
        return false;
      }
    }

    /* copy the stream parameters to the muxer */
    ret = avcodec_parameters_from_context(output_stream->codecpar, encode_ctx);
    if (ret < 0) {
      DLOG_ERROR("avcodec_parameters_from_context failed:{}",
                 errno_to_str(ret));
      return false;
    }
    ...
    if (ret < 0) {
      DLOG_ERROR("avformat_write_header failed:{}", errno_to_str(ret));
      return false;
    }
    ...
    if (ret < 0) {
      DLOG_ERROR("av_frame_get_buffer failed:{}", errno_to_str(ret));
      return false;
    }
  ...
  bool write_frame(const cv::Mat &frame_mat) {
    if (!has_open()) {
      DLOG_ERROR("writer is not opened");
      return false;
    }
    ...
    if (frame_mat.type() == CV_8UC3) {
      ...
      resized_mat =
          ::deepir::math::mat(frame_mat).convert_to(CV_8UC3).get_cv_mat();
    ...
    if (!sws_ctx) {
      DLOG_ERROR("sws_getCachedContext failed");
      return false;
    }
    ...
    if (ret != 0) {
      DLOG_ERROR("avcodec_send_frame failed:{}", errno_to_str(ret));
    ...
    while (true) {
      ...
      if (ret == AVERROR(EAGAIN)) {
        ...
        DLOG_ERROR("avcodec_receive_packet failed:{}", errno_to_str(ret));
        return false;
      }
      ...
      if (ret < 0) {
        DLOG_ERROR("av_write_frame failed:{}", errno_to_str(ret));
        return false;
      }
    }
    return true;
  ...
  void close() {
    if (output_ctx) {
      ...
      if (ret != 0) {
        DLOG_ERROR("av_write_trailer failed:{}", errno_to_str(ret));
      }
    }
...
protected:
  ...
  bool write_package(AVPacket &pkg) {
    ...
    av_packet_rescale_ts(&pkg, encode_ctx->time_base, output_stream->time_base);
