...
namespace cyy::naive_lib::video {
  ...
  public:
    ...
    //! \brief 获取视频帧率
    //! \note 如果无法获取视频帧率，則返回空
    [[nodiscard]] virtual std::optional<std::array<size_t, 2>>
    get_frame_rate() = 0;
    ...
    //	      first<0 失敗
    //	如果first<=0，返回空内容
    [[nodiscard]] virtual std::pair<int, frame> next_frame() = 0;

    //! \brief jump to a frame
    [[nodiscard]] virtual bool seek_frame(size_t frame_seq) = 0;
  };
} // namespace cyy::naive_lib::video
