...
std::string GBK_to_UTF8(const std::string &str) {
  ...
  wcnt = MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, utf16_str.get(), wcnt);
  if (wcnt == 0) {
    LOG_ERROR("MultiByteToWideChar failed");
    return "";
  }
...
std::string UTF8_to_GBK(const std::string &str) {
  ...
  //先计算所需要的buffer大小
  auto cnt =
      WideCharToMultiByte(CP_ACP, 0, reinterpret_cast<LPCWCH>(utf16_str.get()),
                          wcnt, nullptr, 0, nullptr, nullptr);
  ...
  cnt =
      WideCharToMultiByte(CP_ACP, 0, reinterpret_cast<LPCWCH>(utf16_str.get()),
                          wcnt, gbk_str.get(), cnt, nullptr, nullptr);
