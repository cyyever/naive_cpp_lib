...
std::optional<size_t> write(int fd, const void * data,
                            size_t data_len) {
  size_t write_cnt = 0;
  ...
  while (data_len != 0) {
    ...
    if (saved_errno == EAGAIN || saved_errno == EWOULDBLOCK) {
      ...
      LOG_ERROR(
	   "write failed:{}" , ::cyy::cxx_lib::util::errno_to_str(saved_errno));
      return {};
    }
  }
  return {write_cnt};
...
std::optional<std::vector<std::byte>> read(int fd, size_t max_read_size) {
  ...
  while (max_read_size) {
    ...
    if (saved_errno == EAGAIN || saved_errno == EWOULDBLOCK) {
      ...
      LOG_ERROR( "read failed:{}" , ::cyy::cxx_lib::util::errno_to_str(saved_errno));
      return {};
    }
  }
  return {data};
