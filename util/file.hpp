...
namespace cyy::naive_lib::io {
  ...
  //! \brief 读取文件内容
  [[nodiscard]] bool get_file_content(const std::filesystem::path &file_path,
  ...
  write(const std::filesystem::path &file_path, const void *data,
        size_t data_len);

  //! \brief 寫入指定數據到文件
  //! \return
  //! 如果fd是阻塞的，直到所有數據寫入成功才返回，否則返回實際寫入的數據量。如果系統調用失敗，直接返回失敗，不返回之前部分寫入的數據量
  [[nodiscard]] std::optional<size_t> write(int fd, const void *data,
                                            size_t data_len);

  //! \brief 读取文件，最多讀取max_read_size個字節
  //! \return 是否失败
  [[nodiscard]] bool read(int fd, std::vector<std::byte> &buf,
                          const std::optional<size_t> &max_read_size_opt = {});

  //! \brief 读取文件，最多讀取max_read_size個字節
  //! \return 實際讀取到的數據
  [[nodiscard]] std::pair<bool, std::vector<std::byte>>
