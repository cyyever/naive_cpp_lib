...
TEST_CASE("monitor") {
  ...
  REQUIRE(cyy::cxx_lib::process::monitor::init());
  ...
  SUBCASE("kill child") {
    ...
    CHECK(cyy::cxx_lib::process::monitor::signal_monitored_process(child_id,
                                                                   SIGKILL));
    std::this_thread::sleep_for(2s);
    CHECK(!cyy::cxx_lib::process::monitor::monitored_process_exist(child_id));
  }

  SUBCASE("write channel") {
    auto child_id = test_start_process();
    CHECK(cyy::cxx_lib::process::monitor::write_monitored_process(
  ...
  SUBCASE("read channel") {
    ...
    auto data_opt =
        cyy::cxx_lib::process::monitor::read_monitored_process(child_id, 1);
    REQUIRE(data_opt);
