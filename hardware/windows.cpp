...
std::set<std::string> mac_address() {
  ...
  for (size_t i = 0; i < 2; i++) {
    pAddresses =
        new IP_ADAPTER_ADDRESSES[outBufLen / sizeof(IP_ADAPTER_ADDRESSES) + 1];
    ...
    dwRetVal =
        GetAdaptersAddresses(AF_UNSPEC, flags, nullptr, pAddresses, &outBufLen);
...
std::set<std::string> ipv4_address() {
  ...
  for (auto pCurrAddresses = pAddresses; pCurrAddresses;
    ...
    for (auto address = pCurrAddresses->FirstUnicastAddress; address != nullptr;
