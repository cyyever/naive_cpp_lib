...
public:
  ...
  bool allocate(cv::cuda::GpuMat *mat, int rows, int cols,
    ...
    mat->data = static_cast<decltype(mat->data)>(
        get_pool().alloc( step * rows, 1));
    if (mat->data) {
      mat->step = step;
    } else {
...
public:
  ...
  mat_impl &operator/=(float scalar) {
    ...
    if (location != storage_location::cpu) {
      if (channels() == 1) {
        cv::cuda::divide(gpu_mat, scalar, gpu_mat, 1, -1, get_thread_stream());
      } else {
...
#endif
    {
      if (channels() == 1) {
        ...
        cv::divide(cpu_mat, cv::Scalar(scalar, scalar, scalar), cpu_mat, 1, -1);
      }
      location = storage_location::cpu;
    }
    return *this;
...
//! \brief 复制mat_impl内容到gpu buffer
  ...
  void to_gpu_buffer(float *buf) const {
    ...
    //参考opencv-3.2.0/modules/core/src/cuda/gpu_mat.cu cv::cuda::GpuMat::copyTo
  ...
  mat_impl copy_make_border(int top, int bottom, int left, int right,
    ...
    if (location != storage_location::cpu) {
      ...
      cv::cuda::copyMakeBorder(gpu_mat, tmp, top, bottom, left, right,
                               cv::BORDER_CONSTANT, value, get_thread_stream());
      return {tmp};
    }
#endif
    cv::Mat tmp(cpu_mat.rows + top + bottom, cpu_mat.cols + left + right,
                cpu_mat.type());
    copyMakeBorder(cpu_mat, tmp, top, bottom, left, right, cv::BORDER_CONSTANT,
                   value);
    return {tmp};
  }
...
private:
  ...
  static cv::cuda::Stream &get_host_to_device_stream() {
    static std::shared_ptr<cv::cuda::Stream> to_device_stream =
        std::make_shared<cv::cuda::Stream>(cv::cuda::StreamAccessor::wrapStream(
            cyy::cxx_lib::hardware::cuda::get_copy_to_device_stream().value()));
  ...
  static cv::cuda::Stream &get_device_to_host_stream() {
    static std::shared_ptr<cv::cuda::Stream> to_host_stream =
        std::make_shared<cv::cuda::Stream>(cv::cuda::StreamAccessor::wrapStream(
            cyy::cxx_lib::hardware::cuda::get_copy_to_host_stream().value()));
...
bool mat::operator==(const mat &rhs) const { return (*pimpl) == (*rhs.pimpl); }
...
void mat::to_cpu_buffer(float *buf) const { pimpl->to_cpu_buffer<float>(buf); }
...
std::optional<mat> mat::load(const void *buf, size_t size) {
  //先尝试用opencv读取
  auto cv_mat = cv::imdecode(cv::_InputArray(static_cast<const unsigned char *>(buf), size),
